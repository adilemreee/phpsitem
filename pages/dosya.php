...
$checkThisNotify = $db->query("SELECT * FROM `dosyalar` WHERE hash = '$get'");
...
while ($notifyData = $checkThisNotify->fetch()) {
	...
	$ucret = $notifyData['ucret'];
	$content = $notifyData['content'];
	$link = $notifyData['link'];
	$virus = $notifyData['virus'];
	$password2 = $notifyData['password'];
...
$updater = $db->exec("UPDATE dosyalar SET view = $view + 1 WHERE hash = '$get'");
	...
	<div class="d-flex flex-column flex-root">
		<div class="page d-flex flex-row flex-column-fluid">
			...
			if (isset($_POST['commentSend'])) {
				...
				$checkComment = $db->query("SELECT * FROM `dosya_comments` WHERE user_hash = '$post_user_hash' AND hash = '$get'");
				...
				if (empty($comment)) {
					...
					if ($insert == true) {
						$sql = "INSERT INTO `dosya_comments` (user_hash, date_text, preview, hash) VALUES (:user_hash, :date_text, :preview, :hash)";
						$stmt = $db->prepare($sql);
								...
								<div class="flex-lg-row-fluid ms-lg-7 ms-xl-10">
									<div class="card">
										<div class="card-header align-items-center py-5 gap-5">
											<div class="d-flex">
												<a href="dosyalar" class="btn btn-sm btn-icon btn-clear btn-active-light-primary me-3" data-bs-toggle="tooltip" data-bs-placement="top" aria-label="Back" data-bs-original-title="Back" data-kt-initialized="1">
											...
											<div data-kt-inbox-message="message_wrapper">
												...
												<div class="collapse fade show" data-kt-inbox-message="message">
													<div class="py-5 pb-0">
														...
														<p style="color:#b0b4ba;">
															Konu Görüntülenme Sayısı : <?= $view; ?>
														</p>
														<p>
															Saygılarımızla,
														</p>
													...
													<?php
                                        				...
                                        				if ($ucret == "ucretsiz") {
                                            				echo '<a target=”_blank” rel="nofollow" href="' . $link . '" class="btn btn-danger btn-sm"><i class="fa fa-download"></i> İndir</a>';
                                        				} else {
                                          					 echo '<a target=”_blank” rel="nofollow" href="'. $link . '" class="btn btn-info btn-sm"><i class="fa fa-download"></i> Satın Al</a>';
                                        				}

				                                    ?>
													
													<a rel="nofollow" target=”_blank” href="<?= $virus; ?>" class="btn btn-secondary btn-sm"><i class="fa fa-eye"></i> VirusTotal</a>
												</div>
											</div>

											<?php

											$query = "SELECT * FROM `dosya_comments` WHERE hash = '$get' ORDER BY id DESC LIMIT 5";
											$stmt = $db->query($query);
											$count = $stmt->rowCount();
