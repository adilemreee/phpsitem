	...
	<div class="d-flex flex-column flex-root">
		<div class="page d-flex flex-row flex-column-fluid">
			...
			if (isset($_POST['add'])) {
				...
				$post_ucret = htmlspecialchars(strip_tags($_POST['ucret']));
				$post_miktar = htmlspecialchars(strip_tags($_POST['miktar']));
				$post_link = htmlspecialchars(strip_tags($_POST['link']));
				$post_virus = htmlspecialchars(strip_tags($_POST['virus']));
				$post_password = htmlspecialchars(strip_tags($_POST['password']));
				...
				if ($post_title == "" || $post_content == "" || $post_type == "") {
					...
					$record->bindValue(':message', $username . " " . $post_title . " başlıklı dosya paylaştı.");
					$record->bindValue(':icon', "fa fa-file");
					$record->bindValue(':user_hash', $_SESSION['GET_USER_SSID']);
					$record->bindValue(':hour', date('H:i'));
					$record->execute();

					$sql = "INSERT INTO `dosyalar` (ucret, miktar, title, content, type, view, history, hash, user_hash, link, virus, password, admin) VALUES (:ucret, :miktar, :title, :content, :type, :view, :history, :hash, :user_hash, :link, :virus, :password, :admin)";
					$stmt = $db->prepare($sql);
					$stmt->bindParam(':ucret', $post_ucret);
					$stmt->bindParam(':miktar', $post_miktar);
					...
					$stmt->bindParam(':link', $post_link);
					$stmt->bindParam(':virus', $post_virus);
					$stmt->bindParam(':password', $post_password);
					$stmt->bindParam(':admin', $username);
					$stmt->execute();

					echo "<script>toastr.success('Dosya başarıyla eklendi.');</script>";

                    $telegramMessage = "📄️️ <b>Yeni Dosya</b> 📄♂️
                    👤 <b>Ekleyen Kullanıcı:</b> <code>$username</code>
                    📄 <b>Dosya Başlığı:</b> <code>$post_title</code>
                    📄 <b>Eklenen Dosya Konusu:</b> <code>$post_content</code>
                    📄 <b>Eklenen Dosya Linki:</b> <code>$post_link</code>
                    📄 <b>Eklenen Dosya Şifresi:</b> <code>$post_password</code>
						...
						<div id="kt_content_container" class="container-xxl ">
							<div class="card">
								<div class="card-header border-0 pt-5">
									<div class="alert alert-dismissible bg-light-primary border border-primary  d-flex flex-column flex-sm-row w-100 p-5 mb-0">
										...
										<div class="d-flex flex-column pe-0 pe-sm-10">
											...
											<span class="text-gray-800">Dosyaları bu sayfadan ekleyebilirsiniz.</span>
										</div>
								...
								<div class="card-body mt-5 pt-0">
									<?php
									$query = $db->query("SELECT * FROM `dosyalar` WHERE hash = '$get'");
									...
									<form action="" method="POST" autocomplete="off">
										...
										<div class="form-group mt-5">
											...
											<select name="ucret" class="form-control form-control-solid">
										...
										<div class="form-group mt-5">
											<label class="required">Ücret Miktarı(TL)</label>
											<div style="padding: 3px;"></div>
											<input type="text" name="miktar" placeholder="ücret" class="form-control form-control-solid" value="<?= $title; ?>">
										</div>

										<div class="form-group mt-5">
											<label class="required">Dosya Başlığı</label>
											<div style="padding: 3px;"></div>
											<input type="text" name="title" placeholder="Başlık" class="form-control form-control-solid" value="<?= $title; ?>">
										</div>

										<div class="form-group mt-5">
											<label class="required">Dosya İçerik Açıklaması</label>
											<div style="padding: 3px;"></div>
											<textarea id="textarea" type="text" name="content" class="form-control form-control-solid"><?= $content; ?></textarea>
										</div>

										<div class="form-group mt-5">
											<label class="required">Dosya Şifresi</label>
											<div style="padding: 3px;"></div>
											<input type="text" name="password" placeholder="Dosya Rar Şifresi" class="form-control form-control-solid" value="<?= $title; ?>">
										</div>

										<div class="form-group mt-5">
											<label class="required">İndirme Link</label>
											<div style="padding: 3px;"></div>
											<input type="text" name="link" placeholder="İndirme Linki" class="form-control form-control-solid" value="<?= $title; ?>">
										</div>

										<div class="form-group mt-5">
											<label class="required">Virus Total</label>
											<div style="padding: 3px;"></div>
											<input type="text" name="virus" placeholder="Virus Total Linki" class="form-control form-control-solid" value="<?= $title; ?>">
										</div>
										...
										<div class="form-group mt-5">
											...
											<select name="type" class="form-control form-control-solid">
												...
												<option value="database">veri</option>
										...
										<button type="submit" name="add" class="btn btn-light-primary">Dosya Ekle</button>
