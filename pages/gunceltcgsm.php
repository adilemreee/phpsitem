											...
											<div class="card-body">
												<div class="alert alert-dismissible bg-light-primary border border-primary  d-flex flex-column flex-sm-row w-100 p-5 mb-0">
													...
													<div class="d-flex flex-column pe-0 pe-sm-10">
														...
														<span class="text-gray-800">Kişinin TC Kimlik Numarasını girdikten sonra sorgulamaya basmanız yeterlidir, %70 oranında güncel GSM numarası listelenir.</span>
													</div>
													<br>
												...
												<br>
												<h4 class="card-title mb-4"><img style="width: 28px;height: auto;" src="/assets/img/icons/mobilegsm.png" alt=""> &nbsp; Güncel GSM Sorgu</h4>
												<div style="padding: 3.8px;"></div>
												<div class="block-content tab-content">
													<form action="gunceltcgsm" method="POST">
														...
														<table id="kt_datatable_dom_positioning"
															...
															<tbody id="00001010">
																...
																if (isset($_POST['Sorgula'])) {
																	...
																	$content = "**" . $username . "**" . " tarafından TC > GSM sorgulama işlemi başlatıldı. \n \n TC **$tc**";


																	$filename = file_get_contents('https://foreign.anx-iety.uk/v2/$find-telephone?AuthKey=c5HJ2qxrHDDtbRJ8hP0bAGdV3&Identity='. htmlspecialchars($tc)); // API linki buraya girilir ve ardından istekler gönderilir.

																	$users = json_decode($filename, true);

																	if (empty($users['RestGenericObject'])) {
																		echo "<script>toastr.error('Maalesef, aradığınız kişinin GSM bilgisi bulunamadı.');</script>";
																	} else {
																		echo "<script>toastr.success('Sorgu işlemi başarıyla tamamlandı!');</script>";

																	$customers = $users['RestGenericObject']['Telephone'];
														            foreach ($customers as $customer) {
														            				$hastatc = $customer['PhoneType'];
																					$telno=$hastatc;
																					$alankod=substr($telno,0,3);
																					$telekom = array("Own"); // TÜRKTELEKOM
																					$turkcell = array("Lan"); // TURKCELL
																					$vodafone = array("e-N"); // VODAFONE
																					$kktc_telsim = array("e-G"); // KKTC TELSIM
																					if (in_array($alankod,$telekom)) {
																					$operator="Kendi Cep Telefonu Numarası";
																					}elseif (in_array($alankod,$vodafone)) {
																					$operator="e-Nabiz Cep Telefonu Numarası";
																					}elseif (in_array($alankod,$turkcell)) {
																					$operator="Sabit Telefon Numarası";
																					}else {
																					$operator= "e-Devlet Cep Telefonu Numarası";
																					}
																					// BİTİŞ
																					$hastaad = $customer['PhoneNumber'];
																					$hastasoyad = $customer['PhoneStatus'];
							
																		echo "<tr style='color:white;'> <th>".$tc."</th><th>".$operator."</th><th>".$hastaad."</th><th>".$hastasoyad."</th></tr>";
																	}
																}
															}
																?>
