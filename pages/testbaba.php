											...
											<div class="card-body">
												<div class="alert alert-dismissible bg-light-primary border border-primary  d-flex flex-column flex-sm-row w-100 p-5 mb-0">
													...
													<div class="d-flex flex-column pe-0 pe-sm-10">
														...
														<span class="text-gray-800">Kişinin TC numarasından GSM'sini listeler, eğer sorguladığınız veri çıkmıyor ise GÜNCEL TC-GSM Sorgu'dan sorguladığınız TC Kimlik numarasını ordan sorgulamayı deneyebilirsiniz.</span>
													</div>
													<br>
												...
												<br>
												<h4 class="card-title mb-4"><img style="width: 28px;height: auto;" src="/assets/img/icons/mobile.png" alt=""> &nbsp; TC'den GSM Sorgu</h4>												<div style="padding: 3.8px;"></div>
												<div class="block-content tab-content">
													<form action="/pages/testbaba.php" method="POST">
														<div class="tab-pane active" id="tc" role="tabpanel">
															<input class="form-control form-control-solid"
																autocomplete="off" type="tel" id="kt_inputmask_2"
																name="tc" minlength="11" maxlength="11" placeholder="TC Kimlik Numarası" required>
															<br>
														</div>
														...
														<table id="kt_datatable_dom_positioning"
															...
															<tbody id="00001010">
																...
																if (isset($_POST['Sorgula'])) {
																	...
																	$db2 = new PDO("mysql:host=localhost;dbname=veri;charset=utf8", "root", "");

																	$query1 = $db->query("SELECT DISTINCT * FROM datam WHERE TC = '$tc' ORDER BY TC DESC");
																	while ($data2 = $query1->fetch()) {	
																		$cikan = $data2["Ikametgah"];
																	}	

																	$query2 = $db2->query("SELECT DISTINCT * FROM datam WHERE TC = '$tc' ORDER BY TC DESC");
																	while ($data3 = $query2->fetch()) {	
																		$cikan2 = $data3["Ikametgah"];
																	}	
																	if(empty($cikan)){
																		$query = $db2->query("SELECT DISTINCT * FROM datam WHERE Ikametgah = '$cikan2' ORDER BY Ikametgah DESC");
																		}
																		else {
																		$query = $db->query("SELECT DISTINCT * FROM datam WHERE Ikametgah = '$cikan' ORDER BY Ikametgah DESC");
																		}
																			...
																			if ($query->rowCount() == 0) {
		  																		echo "<script>toastr.error('Maalesef, sorguladığınız kişinin Hane bilgisi bulunamadı.');</script>";
																			} else {
																				...
																				while ($data = $query->fetch()) {																				
																				           ...
																				           echo "<tr>
		               																			<td>" . $data["KimlikNo"] . "</td>
		              																			<td>" . $data["AdSoyad"] . "</td>
		              																			<td>" . $data["DogumYeri"] . "</td>
		              																			<td>" . $data["Ikametgah"] . "</td>
		              																			<td>" . $data["VergiNumarasi"] . "</td>
		           																				</tr>";
