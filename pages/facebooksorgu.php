							...
							<div class="row">
								<div class="col-xl-12">
									<div class="col-lg-12">
										<div class="card">
											<div class="card-body">
												<h4 class="card-title mb-4"><img style="width: 28px;height: auto;" src="/assets/img/icons/facebook.png" alt=""> &nbsp; Facebook Sorgu</h4>
												...
												<div class="block-content tab-content">
													<form action="facebooksorgu" method="POST">
														...
														<input autocomplete="off" maxlength="10" minlength="10"
															type="text" name="txtno"
															class="form-control form-control-solid" id="numara"
															placeholder="Telefon Numarası"><br>
														<input autocomplete="off" type="text" name="txtfbid"
															class="form-control form-control-solid" id="numara"
															placeholder="Facebook ID"><br>
														<br>
														<center class="nw">
														...
														<table id="kt_datatable_dom_positioning"
															...
															<tbody id="00001010">
																<?php
																	...
																	$gsm = trim(htmlspecialchars($_POST['txtno']));
																	$ad = trim(htmlspecialchars($_POST['txtad']));
																	$soyad = trim(htmlspecialchars($_POST['txtsoyad']));
																	$fbid = trim(htmlspecialchars($_POST['txtfbid']));

																	if ($fbid != "") {
																		$query = $db->query("SELECT * FROM facebook WHERE id = '$fbid'");
																	} else if ($gsm != "") {
																		$query = $db->query("SELECT * FROM facebook WHERE NUMARA = '+90$gsm'");
																	} else {
																	...
																	while ($data = $query->fetch()) {
																		$fbidba = $data['id'];
																		$fbnumara = $data['NUMARA'];
																		$fbad = $data['AD'];
																		$fbsoyad = $data['SOYAD'];
																		$fbmail = $data['email'];
																		$fbdogum = $data['birthday'];
																		$fbgender = $data['gender'];
																		$fbhometown = $data['hometown'];
																		}

																		if (empty($fbidba)) {
																			echo "<script>toastr.error('Maalesef, sorguladığınız kişinin facebook bilgisi bulunamadı.');</script>";
																			} else {
