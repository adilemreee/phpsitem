...
<body id="kt_body" class="aside-enabled">
	<div class="d-flex flex-column flex-root">
		<div class="page d-flex flex-row flex-column-fluid">
			...
			if (isset($_POST['notify_delete'])) {

				$hash = htmlspecialchars(strip_tags($_POST['hash']));

				$sql = $db->query("DELETE FROM news WHERE hash = '$hash'");
					...
					<div class="post d-flex flex-column-fluid" id="kt_post">
						<div id="kt_content_container" class="container-xxl ">
							<div class="card">
								<div class="card-header border-0 pt-5">
									<div class="alert alert-dismissible bg-light-primary border border-primary  d-flex flex-column flex-sm-row w-100 p-5 mb-0">
										...
										<div class="d-flex flex-column pe-0 pe-sm-10">
											<h5 class="mb-1">Bildirim Yönetimi</h5>
											<div style="padding: 1px;"></div>
											<span class="text-gray-800">Paylaştığımız duyuruları bu platform üzerinden yönetebilirsiniz.</span>
										</div>
								...
								<a href="add-notify" class="btn btn-light-primary">Yeni Bildirim Ekle</a>
										...
										<tbody>
											...
											?>
													<tr>
														<td><?= $m++; ?></td>
														<td><?= $data['title']; ?></td>
														<td><span class="badge badge-light-<?= $color; ?>"><?= $result; ?></span></td>
														...
														<td><?= base64_decode($user_data['username']); ?></td>
														<td>
															<div class="d-flex">

																<a href="view?hex=<?= $data['hash']; ?>" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm me-1">
																	<i class="ki-duotone ki-eye fs-2"><i class="path1"></i><i class="path2"></i><i class="path3"></i></i> </a>

																<a href="edit-notify?hex=<?= urlencode(base64_encode($data['hash'])); ?>" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm me-1">
																	<i class="ki-duotone ki-pencil fs-2"><span class="path1"></span><span class="path2"></span></i> </a>

																<form action="" method="POST" id="deleteForm">
																	<input type="hidden" name="hash" value="<?= $data['hash']; ?>">
																	<button type="submit" name="notify_delete" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm" id="deleteButton">
